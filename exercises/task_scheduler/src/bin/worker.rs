...
fn main() {
    let args = clap::Command::new("Parallel Task worker")
        ...
        .version("0.1")
        .args([Arg::new("task_file").index(1).required(true)])
        .get_matches();
...
fn run_task(task_file: &str, done: &mut bool) -> Result<(), Box<dyn error::Error>> {
    let fp =
        fs::File::open(task_file).expect(format!("Cannot open task file:{}", task_file).as_str());
    ...
    let tmp_file = fs::OpenOptions::new()
        ...
        .create(true)
        .open(tmp_file_name)
        .expect(format!("Cannot open file tmp:{}", tmp_file_name).as_str());
    ...
    fs::rename(tmp_file_name, task_file)
        .expect(format!("Cannot rename {} to {}", tmp_file_name, task_file).as_str());
    ...
    let fields: Vec<String> = first_line
        .trim_end()
        .split_whitespace()
    ...
    if fields.len() != 2 {
        return Err(Box::new(Message::Err(format!(
            "Bad line format: {}",
            first_line
        ))));
    }
